                  ...
                  int CPReflexOn)
{
  ...
  if (p->time[0] < S_INT) {
    if ((S_cum_dt+dt) < S_GRAN) {
      ...
      S_cum_abp +=  (p->x[0] + p->tilt[0])*(S_GRAN - S_cum_dt);
      // S_cum_rap +=  (p->x[15] - p->x[24])*(S_GRAN - S_cum_dt);
	...
	I_cum_rap += rap_bins[i];
      ...
      /*
	if (fabs(I_cum_abp-(*theta).vec[0]) < 5.0)
	s_abp = 0.0;
	else 
	s_abp = atan((I_cum_abp-(*theta).vec[0]) / (*theta).vec[1]) * (*theta).vec[1];
      */

      s_abp = atan((I_cum_abp-(*theta).vec[0]) / (*theta).vec[1])
	* (*theta).vec[1];
      //      s_abp = (I_cum_abp-(*theta).vec[0]);
      s_rap = atan((I_cum_rap-(*theta).vec[15]) / (*theta).vec[16])
	* (*theta).vec[16];
      ...
      //       S_cum_abp =  (p->x[2])*S_cum_dt;
       S_cum_abp =  (p->x[0] + p->tilt[0])*S_cum_dt;
      // S_cum_rap =  (p->x[15] - p->x[24])*S_cum_dt;
  ...
  (*r).resistance[0]  = (*theta).vec[53] + (*theta).vec[4]*alpha_resp + 
    (*theta).vec[17]*alphav_resp;
  //(*r).resistance[1]  = (*theta).vec[55] + (*theta).vec[5]*alpha_resp + 
   // (*theta).vec[18]*alphav_resp;     
  (*r).resistance[2]  = (*theta).vec[57] + (*theta).vec[6]*alpha_resp + 
    (*theta).vec[19]*alphav_resp; 
  (*r).resistance[3]  = (*theta).vec[59] + (*theta).vec[7]*alpha_resp + 
    (*theta).vec[20]*alphav_resp;     
  ...
  Vupold = (*r).volume[0]; Vkold = (*r).volume[1];
  Vspold = (*r).volume[2]; Vllold = (*r).volume[3];

  (*r).volume[0] = (*theta).vec[77] + (*theta).vec[8]*alpha_respv +
    (*theta).vec[21]*alphav_respv;
  p->x[3] += (Vupold - (*r).volume[0]) / (*theta).vec[36];

  (*r).volume[1] = (*theta).vec[78] + (*theta).vec[9]*alpha_respv + 
    (*theta).vec[22]*alphav_respv;
  p->x[8] += (Vkold - (*r).volume[1]) / (*theta).vec[37];

  (*r).volume[2] = (*theta).vec[79] + (*theta).vec[10]*alpha_respv +
    (*theta).vec[23]*alphav_respv;
  p->x[10] = tan(PI*(Vsp+Vspold-(*r).volume[2])/2.0/(*theta).vec[71]) / PI /
    (*theta).vec[38]*2.0*(*theta).vec[71]+p->x[21];

  (*r).volume[3] = (*theta).vec[80] + (*theta).vec[11]*alpha_respv +
    (*theta).vec[24] * alphav_respv;
  p->x[12] = tan(PI*(Vll+Vllold-(*r).volume[3])/2.0/(*theta).vec[72]) / PI /
    (*theta).vec[39]*2.0*(*theta).vec[72]+p->x[22];
}
