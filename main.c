...
// rewritten by Yan Zhang (2013) for separate ouput files
void write_sim_ptr(Output_vector *out, int my_rank, int k)
{
  ...
  if (my_rank == 0)
    {
		(void) sprintf(buf, "Output_Sampling_Data_per_%d_secs.dat", 1);
		samples= fopen(buf, "w");
		fprintf(samples, "TIME HR MAP SV CVP SAP DAP\n");
		for (i=0; i < N_SAMPLES; i= i+k)
			fprintf(samples, "%e %e %e %e %e %e %e\n", T_SAMP*i, out->hr[i], out->map[i], out->sv[i], 
			out->cvp[i], out->sap[i], out->dap[i]);
		fclose(samples);
