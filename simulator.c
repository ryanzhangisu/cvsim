		   ...
		   int tiltTestOn, double tiltStartTime, double tiltStopTime)
{
 ...
 // double days = 0.0;
  ...
  // The following definitions are used in body-fluid homeostasis function (Yan Zhang, 2013)
  
	/* Definition of different parameter arrays */
	double cp[13] = {0.0}; 		// cardiac parameters
	double rp[8] = {0.0};			// renal pressure
	//double r[8] = {0.0};			// resistance
	double qw[7] = {0.0};			// water flow rate
	double qs[8] = {0.0};			// sodium flow rate
	double c[8] = {0.0};			// concentrations
	double cn[10] = {0.0};			// normal value of concentrations
	double ref[7] = {0.0};			// reflex control 
	double coef[6] = {0.0};			// coefficients
	double eff[17] = {0.0};			// effect parameters
	double vol[3] = {0.0};			// volumes
	double tao[3] = {0.0};			// time constants
	double vas[3] = {0.0};			// vascularity
	double dydt[8] = {0.0};  		// derivatives of dependent variables
	
	double sodin = 0.126; //sodium intake mEq/min
	double waterin = 0.0;	// (additional) water intake L/min (Use additional because of the thirst-driven water intake equation in the renal function model)
  ...
  eqns_ptr(&pressure, a, &reflex, qw, tiltTestOn, tiltStartTime, tiltStopTime); 

  makeImp(&imp, a);                    // Set up the impulse resp. arrays

  numerics(&pressure, &reflex, out, a);
  
  /* Initialization for the renal and body fluid model */
  	renal_init(&pressure, cp, rp, qw, qs, c, cn, ref, coef, eff, vol, tao, vas);
	renal_eqns(cp, rp, qw, qs, c, cn, ref, coef, eff, vol, tao, vas, dydt, sodin, waterin);
	//renal_write(0, n, t, p, r, eff, cn, qw, qs, c, ref, vol, vas);
	//double nn = 0.0; // number of repeated water drinking
	//double time_interval = 0.0; // time period between two drinks
  while (pressure.time[0] < 60.*24.*5.) {
	printf("Now working at time %e, Time step size is %e\n", pressure.time[0], hdid); 
	
	if(pressure.time[0]>60.*24.*2.) waterin = 0.0051667;
	else waterin = 0.0;
	/*if(pressure.time[0] > 60.*24. && nn < 30.0*3.0){
		if(time_interval < 60.){
			if(pressure.time[0] > 60.*24. + 60.*nn && pressure.time[0] < (60.*24.+ 60.*nn + 28.)){ 
			waterin = 0.0143;
			}
			else {
			waterin = 0.001;
	...
	/*if(pressure.time[0] > 60.*24.*3.){
		waterin = 0.00378;
		if(pressure.time[0] > 60.*24.*6.){
			days = (pressure.time[0] - 60.*24.*6.) / (60.*24.);
			a->vec[49] = 2.0 * (1.25 - 0.25 * exp(-days/1.5));
			a->vec[50] = 1.64 * (1.5 - 0.5 * exp(-days/1.753)); 
			a->vec[43] = 3.33 * (1.15 - 0.15 * exp(-days/2.8));
			a->vec[44] = 1.35 * (1.975 - 0.9706 * exp(-days/1.753));
		}
	}
	else waterin = 0.0;*/
    ...
    rkqc_ptr(&pressure, &reflex, a, htry, 0.001, yscale, &hdid, &hnext, 
			qw, tiltTestOn, tiltStartTime, tiltStopTime);
    ...
    eqns_ptr(&pressure, a, &reflex, qw, tiltTestOn, tiltStartTime, tiltStopTime);
    // The following function calculates body fluid homeostasis based current MAP, RAP and sodium water intake
    // Return a value of delta_q which modifies the venous return flow.
    
	renal_function(&pressure, &reflex, cp, rp, qw, qs, c, cn, ref, coef, eff, vol, 
			tao, vas, dydt, sodin, waterin, hdid);
    ...
    if (pressure.time[0] > 60.*24. && pressure.time[0] < (60.*24. + 20.) ) write_parameter(&pressure, &reflex, 1);
    if (pressure.time[0] > 60.*24.*4. && pressure.time[0] < (60.*24.*4. + 20.) ) write_parameter(&pressure, &reflex, 2);
  }
...
//write computed parameters to files (Yan Zhang, 2013) 
void write_parameter(Data_vector *pressure, Reflex_vector *reflex_y, int x)
{
	int i;
	char buf[256];
	
	FILE *output_p;
	(void) sprintf(buf, "time_series_pressure_%d.dat", x);
	output_p = fopen(buf, "a");
	fprintf(output_p, "%e ", pressure->time[0]);
	for (i=0; i<21; i++) 
	{
		fprintf(output_p, "%e ", pressure->x[i]);
	}
	fprintf(output_p, "\n");
	fclose(output_p);
	
	FILE *output_q;
	(void) sprintf(buf, "time_series_flow_%d.dat", x);
	output_q = fopen(buf, "a");
	fprintf(output_q, "%e ", pressure->time[0]);
	for (i=0; i<21; i++) 
	{
		fprintf(output_p, "%e ", pressure->q[i]);
	}
	fprintf(output_p, "\n");
	fclose(output_q);

   	FILE *output_v;
   	(void) sprintf(buf, "time_series_volume_%d.dat", x);
	output_v = fopen(buf, "a");
	fprintf(output_v, "%e ", pressure->time[0]);
	for (i=0; i<21; i++) 
	{
		fprintf(output_p, "%e ", pressure->v[i]);
	}
	fprintf(output_p, "\n");
	fclose(output_v);
	
	FILE *output_hr;
	(void) sprintf(buf, "time_series_heart_rate_%d.dat", x);
	output_hr = fopen(buf, "a");
	fprintf(output_hr, "%e ", pressure->time[0]);
	fprintf(output_hr, "%e\n", reflex_y->hr[2]);
	fclose(output_hr);
}
